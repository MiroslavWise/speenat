...
export const speakerSpecAdd = async (value: IDataReplaceSpec): Promise<ISpecList> => axiosInstance.post(`/speaker/spec/add/`, value)
...
export const specializationsAllList = async (): Promise<ISpecializationsAll[]> => axiosInstance.get(`/specialization/`).then(response => response.data).catch(e => { console.error("USER DATA: ", e) })