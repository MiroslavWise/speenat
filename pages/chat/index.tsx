                                        ...
                                        data && data?.length > 0
                                                ...
                                                data?.map(item => (
                                                        <div
                                                                className="item-chat"
                                                                key={item?.uuid}
                                                                onClick={() => { push(`/chat/${item?.uuid}`, undefined, { shallow: true }) }}
                                                        >
                                                                <Image
                                                                        loader={loadImage}
                                                                        src={replaceHttps(srcImage(isSpeaker ? item?.student?.photo_url : item?.speaker?.photo_url))}
                                                                        alt="photo"
                                                                        height={80}
                                                                        width={80}
                                                                        className="avatar"
                                                                ...
                                                                <div className="description-chat">
                                                                        <p className="full-name-class">{isSpeaker ? item?.student?.full_name : item?.speaker?.full_name}</p>
                                                                        <div className="last-messages"><p>{item?.messages[0]?.text}</p></div>
                                                                        <div className="time-message"><i>{moment.utc(item?.messages?.[0]?.created_at).format("HH:mm DD.MM.YY")}</i></div>
                                                                </div>
                                                        </div>
                                                )) : null
                                }
